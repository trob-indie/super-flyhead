...
@export var skew_amount := 0.3  # new: max skew at outermost columns

func _ready():
	generate_perspective_grid()

func generate_perspective_grid():
	# Precompute consistent column-based scale factors for seam alignment
	...
	for row in range(rows):
		...
		for col in range(columns):
			...
			tile.position = Vector2(x_start + x_offset, y_offset)
			tile.skew = -normalized_offset * skew_amount  # Skew more at sides
			tile.z_index = row
			add_child(tile)
